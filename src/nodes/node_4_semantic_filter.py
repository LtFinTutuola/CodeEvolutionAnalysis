...
def node_4_semantic_filter(state):
    ...
    for commit_hash, commit_hunks in by_commit.items():
        ...
        add_by_sig = {h["signature"]: (k, h) for k, h in additions.items() if k not in matched_add_hashes}
        del_by_sig = {h["signature"]: (k, h) for k, h in deletions.items() if k not in matched_del_hashes}
    ...
    return {"parsed_hunks": final_hunks}
